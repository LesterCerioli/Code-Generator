...
class Config:
    ...
    ckpt_path: str = "ckpt_java_solid.pt"
...
Java: keep classes cohesive; one reason to change.
...
Extend via interfaces and new implementations, not modifying core.

Example:
public interface DiscountRule {
  Money apply(Money amount);
}
public final class SeasonalDiscount implements DiscountRule {
  public Money apply(Money amount) { return amount.multiply(0.10); }
}
""",
...
public final class FIFOQueue<T> implements Queue<T> { /* ... */ }
""",
    """Principle: Interface Segregation
Prefer small focused interfaces.

Example:
public interface Logger { void info(String m); void warn(String m); }
public interface Metrics { void incr(String key); }
""",
    """Principle: Dependency Inversion
Depend on abstractions; inject concrete via constructors/factories.
...
public final class Signup {
  ...
  public Signup(Mailer mailer) { this.mailer = mailer; }
  public void run(String email) { mailer.send(email, "welcome"); }
}
""",
    """Anti-pattern: God class
class App { void run(); void saveUser(); void sendEmail(); }
Fix: extract services and repositories."""
]

# Byte-level
def encode(s: str) -> torch.Tensor: return torch.tensor(list(s.encode("utf-8")), dtype=torch.long)
def decode(t: torch.Tensor) -> str: return bytes(t.tolist()).decode("utf-8", errors="ignore")
...
class SelfAttention(nn.Module):
    def __init__(self, C, H, p): super().__init__(); self.H=H; self.key=nn.Linear(C,C,False); self.query=nn.Linear(C,C,False); self.value=nn.Linear(C,C,False); self.proj=nn.Linear(C,C); self.drop=nn.Dropout(p)
    def forward(self,x):
        B,T,C=x.size()
        k=self.key(x).view(B,T,self.H,C//self.H).transpose(1,2)
        q=self.query(x).view(B,T,self.H,C//self.H).transpose(1,2)
        v=self.value(x).view(B,T,self.H,C//self.H).transpose(1,2)
